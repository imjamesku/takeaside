...
const TopicDetailPage = ({ serverTopicData, error }: Props) => {
  ...
  const topicFetcher = (key: string) =>
    instance.get(key).then((res) => res.data);
  const { data: topicData, error: swrError, mutate } = useSWR<Topic>(
    `/topics/${id}`,
    topicFetcher,
    { initialData: serverTopicData }
  );
  ...
  if (topicData) {
    return (
      <Layout
        title={`${topicData.question} | imbiased`}
        description={`What's your opinion on it?`}
      >
        <div className={styles.title}>
          <h1>{topicData.question}</h1>
          <span>
            Created by{" "}
            <Link href="/users/[id]" as={`/users/${topicData.userId}`}>
              <a className={styles.handle}>@{topicData.userName}</a>
            </Link>{" "}
            - {moment.utc(topicData.createdAt).fromNow()}
          </span>
        </div>
        <span>Share: </span>
        <CopyToClipboard
          text={`${process.env.NEXT_PUBLIC_FRONTEND_URL}/topic/${id}`}
          onCopy={() => {
            setCopied(true);
            alert("copied");
          }}
        ...
        <div className={styles.bar}>
          <PercentageBar
            leftVotes={topicData.left.count}
            rightVotes={topicData.right.count}
          />
        </div>
        <div className={styles.sides}>
          <div className={styles.left}>
            ...
            <button
              ...
              onClick={() => vote(topicData.left.id)}
            >
              VOTE
            </button>
          </div>
          <div className={styles.right}>
            ...
            <button
              className={styles.vote}
              onClick={() => vote(topicData.right.id)}
            >
              VOTE
            </button>
          </div>
        </div>
        <Comments topicId={topicData.id} />
      </Layout>
    );
  }
  return (
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  try {
    ...
    return {
      props: { serverTopicData: null, error: "Failed to fetch topic data" },
    };
  } catch (error) {
    return {
      props: { serverTopicData: null, error: "Failed to fetch topic data" },
